...
description: >-
  Guides use of the eonik MCP brand-brain: brand + ledger, competitor
  research archive, own-account readouts, deconstruction, memory, and
  receipt-bound brief grounding. Agents read and draft; they never spend.
  Activates when eonik MCP tools are connected or the user asks about
  their ads, competitors, brand, or what to make next.
---

# eonik MCP companion

Use this skill whenever **eonik MCP tools** are in scope. Auth is the host's MCP connection (API key or OAuth). Tools run against **this user's** workspace.

eonik is a **Mac app for finished, on-brand ads**. This skill is the **cloud brand-brain** (research, memory, readouts, briefs). Timeline assembly and export live in the Mac app. **You approve every cut. Nothing here moves budget.**

## Principles (CORE_PRINCIPLES)

1. **Ground first.** Call `get_brand_briefing` or `get_brand_context` + `get_context_ledger` (+ `recall` when the user may have already ruled) before strategic or creative advice. The ledger overrides generic best practice.
...
3. **eonik does the labor; you reason.** Deconstruct, patterns, timeline, craft playbook, brief **grounding** are busywork. Author the brief yourself from `get_brief_grounding`. Prefer that over `generate_creative_brief` (deprecated server author).
...
| What should we consider / slate | `get_recent_slates` → receipts via competitor / deconstruct tools |
| What are competitors running | `get_competitor_channels` → `get_competitor_assets` or `get_my_competitor_ads` / `get_watch_activity` |
| How they make ads (editor pointers) | `get_craft_playbook` (counts over survivors, not causation) |
